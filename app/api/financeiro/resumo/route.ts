...
export async function GET(request: NextRequest) {
  try {
    ...
    // Lancamentos manuais no periodo (apenas manuais, não automáticos de OS/Contratos)
    let lancQuery = `
      SELECT 
    ...
    const chartData = database.prepare(`
      ...
      FROM (
        -- ordens: pagas x pendentes (data_ref = data_execucao)
        SELECT data_execucao as data_ref,
          CASE WHEN liquidado = 1 THEN COALESCE(valor_pago, valor, 0) ELSE 0 END as receita_paga,
          CASE WHEN liquidado = 0 THEN COALESCE(valor, 0) ELSE 0 END as receita_pendente,
          0 as despesa_paga,
        ...
        -- lancamentos manuais: pagas x pendentes (data_lancamento)
