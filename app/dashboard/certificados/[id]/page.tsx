...
export default function CertificadoDetalhesPage() {
  ...
  const fetchCertificado = async () => {
    try {
      setLoading(true)
      const response = await fetch(`/api/certificados/${params.id}`)
      if (response.ok) {
  ...
  const handleDownloadPDF = async () => {
    try {
      const response = await fetch(`/api/certificados/${params.id}/pdf`)
      if (response.ok) {
  ...
  const actionItems = [
    ...
    },
    {
      ...
      onClick: () => router.push(`/dashboard/certificados/${params.id}/edit`)
    },
    {
      label: 'Baixar PDF',
        ...
        <div className="space-y-4">
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              ...
              <Link href={`/dashboard/certificados/${params.id}/edit`}>
