...
export async function createClienteAction(data: Omit<Cliente, 'id' | 'created_at' | 'updated_at'>) {
  ...
  try {
    database
      .prepare(
        `INSERT INTO clientes (id, user_id, razao_social, nome_fantasia, cnpj, endereco, cidade, estado, cep, telefone, email, contato_responsavel, observacoes, ativo)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
      )
      .run(
        id,
    ...
    if (err.code === 'SQLITE_CONSTRAINT_UNIQUE') return { error: 'CNPJ já cadastrado' }
...
export async function updateClienteAction(
  ...
  try {
    const result = database
      .prepare(
        `UPDATE clientes SET razao_social=?, nome_fantasia=?, cnpj=?, endereco=?, cidade=?, estado=?, cep=?, telefone=?, email=?, contato_responsavel=?, observacoes=?, ativo=?, updated_at=datetime('now')
